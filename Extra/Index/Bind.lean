...
def bind (f : α → List β) : {xs : List α} → (i : Index xs) × (Index (f i.val)) → Index (xs.bind f)
  | _::_, ⟨head, j⟩ => append_inl j
  | _::_, ⟨tail i, j⟩ => append_inr (bind f ⟨i, j⟩)

def unbind (f : α → List β) : {xs : List α} → (k : Index (xs.bind f)) → (i : Index xs) × (Index (f i.val))
  | _::_, k =>
...
theorem bind_unbind (f : α → List β) {xs : List α} (k : Index (xs.bind f)) : bind f (unbind f k) = k := by
  induction xs with
  | nil => contradiction
  | cons x xs ih =>
    rw [unbind]
...
theorem bind_eq_iff_eq_unbind (f : α → List β) (i : (i : Index xs) × Index (f i.val)) (j : Index (xs.bind f)) : bind f i = j ↔ i = unbind f j := by
  constructor
  · intro h; rw [←h, unbind_bind]
  · intro h; rw [h, bind_unbind]

theorem unbind_eq_iff_eq_bind (f : α → List β) (i : Index (xs.bind f)) (j : (i : Index xs) × Index (f i.val)) : unbind f i = j ↔ i = bind f j := by
  constructor
  · intro h; rw [←h, bind_unbind]
  · intro h; rw [h, unbind_bind]

def bindEquiv (f : α → List β) (xs : List α) : Equiv ((i : Index xs) × Index (f i.val)) (Index (xs.bind f)) where
  fwd := bind f
  rev := unbind f
...
theorem val_unbind (f : α → List β)  {xs : List α} (i : Index (xs.bind f)) : (unbind f i).snd.val = i.val := by
