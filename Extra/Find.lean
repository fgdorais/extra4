...
protected abbrev any [Find α] (p : α → Bool) := (Find.find? p).isSome

protected abbrev all [Find α] (p : α → Bool) := (Find.find? (!p ·)).isNone

@[simp] theorem any_iff_exists [Find α] (p : α → Bool) : Find.any p ↔ ∃ x, p x :=
